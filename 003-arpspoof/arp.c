                     ...
                     int* arp_flags_out, struct ether_addr* ether_out)
{
	...
	/*
	 * arp info will sotre in arp variable
	 */
	...
	/*
	 * arp_flags will be set if ioctl success
	 */
    if (arp_flags_out)
									 ...
									 const char* trgt_mac, const char* trgt_ip)
{
	...
	ap->ap_hln = ETH_ALEN;
	ap->ap_pln = 4;
	ap->ap_op = htons(op_code); memcpy(ap->ap_sha, ether_aton(sendr_mac)->ether_addr_octet, ETH_ALEN);
